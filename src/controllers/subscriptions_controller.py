...
def created_subscribe(payload: SubscriptionsCreate, db: Session = Depends(get_db)):
    ...
    """
        ...
        id_subscriptions = Subscriptions.id_subscriptions,
        cancelled_id = 0,
        client_id = Subscriptions.client_id,   
        fund_id = Subscriptions.fund_id,
        ...
        message = f"Subscription to fund {payload.fund_id} confirmed for amount {payload.amount}"
        strategies = {
        'Email': Email,
        'SMS': SMS
    }
...
def delete_subscription(subscriptionsid: int, db: Session = Depends(get_db)):
    ...
    subscription = db.query(subscriptions).filter(subscriptions.id_subscriptions == subscriptionsid).first()
    if not subscription:
